#!/usr/bin/env node
// АРХИВНЫЙ СКРИПТ (Ф5): вход (data/v2/zones.json, data/v2/markers.json) удалён из
// репо декомиссией Ф5 — прод два деплоя живёт на v3, v2-эталон отката отслужил.
// Скрипт больше не запускается и не запустится (ENOENT на ZONES_IN/MARKERS_IN);
// оставлен только как документация аффинного фита v2->v3 ниже (коэффициенты,
// контрольные точки, невязка — реальная справочная ценность на случай пересчёта).
//
