...
// tiles/ — локальный симлинк на ~/Downloads/drakkenheim_maphost/tiles_v3 (не в репо,
...
// Теперь — на каждый прогон тестов, автоматически: клон ТЕКУЩЕЙ геометрии/полей data/v3
// (Ф3.5в: источник переключён с data/v2 на data/v3, тот же принцип),
...
async function generateLifecycleFixture() {
  ...
  const zonesReal = JSON.parse(fs.readFileSync(path.join(root, 'data', 'v3', 'zones.json'), 'utf8'));
  const markersReal = JSON.parse(fs.readFileSync(path.join(root, 'data', 'v3', 'markers.json'), 'utf8'));

  const salt = DKCrypto.randomSalt();
  const key = await DKCrypto.deriveKey(LIFECYCLE_PW, salt);

  const zonesOut = { schema: 'dk-map/v3', mapOrientation: 'v3', items: [] };
  ...
  const markersOut = { schema: 'dk-map/v3', mapOrientation: 'v3', items: [] };
...
module.exports = async function globalSetup() {
  ...
  try {
    ...
    throw new Error(
      ...
      'Ожидается симлинк tiles -> .../drakkenheim_maphost/tiles_v3 (см. README).\n' +
